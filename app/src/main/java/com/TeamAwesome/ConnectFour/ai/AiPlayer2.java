...
public class AiPlayer2 {
    ...
    private final BoardLogic2 mBoardLogic2;


    private int mMaxDepth;

    public AiPlayer2(BoardLogic2 boardLogic2) {
        this.mBoardLogic2 = boardLogic2;
    }
    ...
    private Move chooseMove(int player, int opponent,
        ...
        for (int i = 0; i < mBoardLogic2.numCols; i++) {
            if (mBoardLogic2.columnHeight(i) > 0) {
                // add a counter to that column, then check for win-condition
                mBoardLogic2.placeMove(i, player);
                // score this move and all its children
                int score = 0;
                if (mBoardLogic2.checkMatch(i, mBoardLogic2.columnHeight(i))) {
